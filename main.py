...
def save_image(dataset_name, pdf_name):
    file_path = os.path.join(dataset_name, 'pdf', f'{pdf_name}.pdf')
    img = convert_from_path(file_path)[0]
    width, height = img.size
    fname = f'{pdf_name}.png'
    img.save(os.path.join(dataset_name, 'images', fname))
...
def process_vqa_line(dataset_name, split, line1, line2):
    ...
    record_init = {"id": full_id,
                   "input": {"id": full_id, "uid": full_id,
                             "image": save_image(dataset_name, pdf_name)}}
    ...
    for ocr_id in range(ocr_num):
        ...
        for query_id in range(query_num):
            ...
            write_json(dataset_name, record_final)
            write_index_file(dataset_name, pdf_name, record_final["id"], split_name)


def process_vqa_jsonl(dataset_name, split='', line_num=None):
    file1_path = os.path.join(dataset_name, split, 'document.jsonl')
    file2_path = os.path.join(dataset_name, split, 'documents_content.jsonl')

    with open(file1_path, 'r', encoding="utf-8") as file1, open(file2_path, 'r', encoding="utf-8") as file2:
        for i, (line1, line2) in tqdm.tqdm(enumerate(zip(file1, file2))):
            if i == line_num:
                break
            process_vqa_line(dataset_name, split, line1, line2)


def write_json(dataset_name, processed_record):
    file_name = processed_record["id"]
    with open(os.path.join(dataset_name, 'jsons', f'{file_name}.json'), 'w') as json_file:
...
def write_index_file(dataset_name, image_file_name, json_file_name, split_name):
    ...
    with open(index_file_name1, 'a') as index_file1, open(index_file_name2, 'a') as index_file2:
        index_file1.write(
            f"{os.path.join(dataset_name, 'images')}/{image_file_name}\t{os.path.join(dataset_name, 'jsons')}/{json_file_name}\n")
        index_file2.write(
            f"{os.path.join(dataset_name, 'images')}/{image_file_name}\t{os.path.join(dataset_name, 'jsons')}/{json_file_name}\n")


def process_dataset(dataset_name):
    if not os.path.exists(os.path.join(dataset_name, 'jsons')):
        os.mkdir(os.path.join(dataset_name, 'jsons'))
    if not os.path.exists(os.path.join(dataset_name, 'images')):
        os.mkdir(os.path.join(dataset_name, 'images'))

    process_vqa_jsonl(dataset_name, 'train', 5)
    process_vqa_jsonl(dataset_name, 'dev', 2)
    process_vqa_jsonl(dataset_name, 'test', 2)


if __name__ == '__main__':
    process_dataset('docvqa')
